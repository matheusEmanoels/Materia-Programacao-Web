...
public class JWTAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request,
        ...
        try {
            //Ao realizar um HTTP.POST com o conteúdo no formato JSON:
            ...
            //Verifica se o usuário existe no banco de dados, caso não exista uma Exception
            // será disparada
		    ...
		    /* Como o authenticationManager foi configurado na classe WebSecurity e, foi informado o metodo
			  ...
			  disparada Caso ocorra sucesso será chamado o metodo: successfulAuthentication dessa classe */
    ...
    protected void successfulAuthentication(HttpServletRequest request,
        ...
        // o metodo create() da classe JWT é utilizado para criação de um novo token JWT
        String token = JWT.create()
                .withSubject(authResult.getName()) // o objeto authResult possui os dados do
                // usuário autenticado, nesse caso o metodo getName() retorna o username do
                // usuário que foi autenticado no metodo attemptAuthentication.
                //a data de validade do token é a data atual mais o valor armazenado na
                // constante EXPIRATION_TIME, nesse caso 1 dia
                .withExpiresAt( new Date(System.currentTimeMillis() +
                        SecurityConstants.EXPIRATION_TIME) )
                //Por fim é informado o algoritmo utilizado para assinar o token e por
                // parâmetro a chave utilizada para assinatura. O Secret também pode ser alterado
                // na classe SecurityConstants que armazena alguns dados de configuração
                // do Spring Security
                .sign(Algorithm.HMAC512(SecurityConstants.SECRET));
        response.setContentType("application/json");
        response.getWriter().write(new ObjectMapper()
                .writeValueAsString(new AuthenticationResponse(token, new UserResponseDTO(user))));
