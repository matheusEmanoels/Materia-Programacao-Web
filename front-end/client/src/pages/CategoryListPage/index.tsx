    ...
    const onClickRemove = async(id?:number) =>{
        if(id){
            ...
            if(response.status === 204 || response.status === 200){
               await loadData();
               setApiRemoveSuccess("Categoria Removida com sucesso!")
               setTimeout(()=>{
                   setApiRemoveSuccess("")
               }, 2000)
    ...
    return(
        <>
            <main className="container">
                ...
                <Link to="/categories/new" className="btn btn-success">Nova Categoria</Link>
                    ...
                    <tbody>
                        {data.map((category : ICategory) => (
                            <tr key={category.id}>
                                ...
                                <td>{category.name}</td>
                                <td>
                                    <Link to={`/categories/${category.id}`} className="btn btn-info">Editar</Link>
                                </td>
                                <td>
                                    <button className="btn btn-danger" onClick={()=>onClickRemove(category.id)}>Excluir</button>
                                </td>
                            </tr>
                        ))}
                        <tr></tr>
